...
def reset_backtest_singletons():
    """Reset backtest singletons between tests."""
...
def app_client_with_data(app_client: TestClient, populated_store: ParquetStore):
    ...
    backtest_routes._parquet_store = populated_store
    return app_client
...
class TestBotsEndpoint:
    ...
    def test_list_bots(self, app_client: TestClient) -> None:
        """Should return list of available bots."""
        response = app_client.get("/backtest/bots")
...
class TestRunEndpoint:
    ...
    def test_run_returns_run_id(self, app_client: TestClient) -> None:
    ...
    def test_run_invalid_bot_returns_400(self, app_client: TestClient) -> None:
    ...
    def test_run_empty_symbols_returns_422(self, app_client: TestClient) -> None:
...
class TestStatusEndpoint:
    ...
    def test_status_not_found(self, app_client: TestClient) -> None:
        """Should return 404 for unknown run_id."""
        response = app_client.get("/backtest/status", params={"run_id": "nonexistent"})

        assert response.status_code == 404

    def test_status_returns_running(self, app_client: TestClient) -> None:
        ...
        response = app_client.get("/backtest/status", params={"run_id": run_id})
...
class TestResultsEndpoint:
    ...
    def test_results_not_found(self, app_client: TestClient) -> None:
        """Should return 404 for unknown run_id."""
        response = app_client.get("/backtest/results", params={"run_id": "nonexistent"})
...
class TestBacktestIntegration:
    ...
    def test_full_backtest_flow(self, app_client: TestClient, populated_store: ParquetStore) -> None:
        ...
        # Wait a bit for job to complete (in test, runs synchronously or very fast)
        import time
        time.sleep(0.5)

        # Check status
        status_response = app_client.get("/backtest/status", params={"run_id": run_id})
...
class TestSweepEndpoint:
    ...
    def test_sweep_returns_sweep_id(self, app_client: TestClient) -> None:
    ...
    def test_sweep_invalid_bot_returns_400(self, app_client: TestClient) -> None:
        """Should return 400 for invalid bot name."""
        response = app_client.post(
            "/backtest/sweep",
            json={
    ...
    def test_sweep_status_not_found(self, app_client: TestClient) -> None:
        """Should return 404 for unknown sweep_id."""
        response = app_client.get("/backtest/sweep/status", params={"sweep_id": "nonexistent"})
...
class TestNoRunningEventLoopFix:
    ...
    def test_backtest_completes_without_event_loop_error(self, app_client: TestClient, populated_store: ParquetStore) -> None:
        ...
        for _ in range(20):
            time.sleep(0.5)
            s = app_client.get("/backtest/status", params={"run_id": run_id})
        ...
        results_resp = app_client.get("/backtest/results", params={"run_id": run_id})
